...
static int summon_object(y, x, num, typ, good)
int y, x, num, typ;
int32u good;
{
  register int i, j, k;
      ...
      do
	{
	  ...
	  if (in_bounds(j, k) && los(y, x, j, k))
	      ...
	      if (c_ptr->fval <= MAX_OPEN_SPACE && (c_ptr->tptr == 0))
		{
		  ...
		  if (real_typ == 1) {
		    if (good) place_special(j,k,good);
		    else place_object(j, k);
		  } else {
		    if (good) place_special(j,k,good);
		    else place_gold(j, k);
		  }
...
int32u monster_death(y, x, flags, good, win)
int y, x;
...
int monptr, dam;
{
  ...
  if (m_ptr->hp < 0)
	...
	if (!dun_level) {
	  sprintf(tmp, "%s%d", ANGBAND_BONES, c_list[m_ptr->mptr].level);
	} else {
...
/* temp new message */
	  ...
	  if (mon_take_hit(crptr, k) >= 0)
	    {
              if ((c_list[monptr].cdefense & DEMON) ||
                  (c_list[monptr].cdefense & UNDEAD))
      ...
      else if (c_ptr->tptr != 0)
	{
	  ...
	  if (i == TV_VIS_TRAP)		    /* Floor trap    */
	      ...
	      else
		{
		  ...
		  /* make sure we move onto the trap even if
confused */
		  tmp = py.flags.confused;
		  py.flags.confused = 0;
		  move_char(dir, FALSE);
...
int y, x;
{
  ...
  if (test_hit(base_tohit, (int)py.misc.lev,
	...
	{
          if ((c_list[m_ptr->mptr].cdefense & DEMON) ||
              (c_list[m_ptr->mptr].cdefense & DEMON))
	    (void) sprintf(out_val, "You have destroyed %s.", m_name);
          else
	    (void) sprintf(out_val, "You have slain %s.", m_name);
	  msg_print(out_val);
